...
#### 💰 Web Monetization インフラ再整備
- GateHubの本人確認（KYC）再申請を完了。「Identity document」の再アップロード（見開き画像）を行い、現在審査待ち。
- 承認後の `isMonetized` フラグによる「黄金モード」の挙動を確認済み。

#### 🗡️ シナリオ及びUIの拡充
- 「五丈原」「赤壁」「漢中」「官渡」「合肥」の5戦場を新規追加・プロット。
- 勢力表示（蜀・魏・呉・袁）の動的化と、歴史解説ツールチップの実装。
- 地図描画ロジックのリファクタリングにより、今後の拡張性を確保。

### 2026-02-11
- **Web Monetization APIの実装と最適化**:
  - 支払いポインターの連携を完了。ブラウザからの支援イベント（monetizationprogress）を検知する基盤を構築。
- **リアルタイム軍資金システム（Treasury Log）**:
  - EUR単位での高精度カウンターを実装。
  - `localStorage` による永続化に対応し、リロード後も兵糧（残高）を維持する機能を搭載。
  - 本番運用を見据え、初期値を `false` に設定して実際の信号待ち状態へ調整。
- **黄金モード（Golden Mode）の視覚演出**:
  - 支援有効時に背景が黄金色に変化する演出と、専用称号プレートの動的表示を実装。
- **UI/UXの洗練**:
  - 「支援感謝メッセージ」をコンポーネント化（ThankYouToast）。
  - 再レンダリングに影響されない5秒自動消失ロジックと、セッション内一度のみ表示の制御を実装。
  - ヘッダー・感謝メッセージ・兵糧カウンターの三段配置（Z軸・Y軸）の重なりを解消。
- **動的リザルトメッセージ**:
  - 戦闘結果画面に、三國志や春秋戦国時代の逸話を織り交ぜた「天の加護」メッセージをランダム生成するロジックを追加。

### 2026-02-14
- 行軍シミュレーター実装: 成都〜漢中の「40日の絶望」を可視化。
- 官職名鑑（文武両道）: 一品から雑号将軍まで、階級社会をデータ化。
- Home UI統合: 複数の戦略機能を選べる司令部を構築。

#### 次回の開発ロードマップ
1. 名鑑のフィルタリング実装
- 目的: 文官・武官、あるいは「一品」「二品」などの階級ごとに表示を絞り込み、視認性を高める。
- 技術要素: useState で filterCategory を管理し、MILITARY_RANKS.filter() で動的にリストを生成します。

2. 詳細表示モーダルのデザイン
- 目的: タップした官職の由来、当時の権限、歴史的事件などをマニアックに深掘りする。
- 技術要素:
...
- [x] クリアランク機能（基礎実装）: スコアやモネタイズ状態に応じた称号（getPlayerTitle）の表示。
- [ ] 兵糧消費システム（雇用・計略）: 貯まった軍資金（EUR）を消費して、強力な武将を「雇用」したり、クイズ中に「計略（ヒント）」を購入できるショップ機能の構築。
- [ ] 支援者特典の更なる強化: 黄金モード時、クイズのヒント使用回数を増加、または限定エピソードの解放。
- [ ] GateHub APIの最終解決: HostedウォレットでのAPIキー取得方法の特定、またはXRPLウォレットへの完全移行による残高同期の実現。
- [ ] SE/BGMの実装: 支援開始時の「チャリン」というSEや、黄金モード専用の荘厳なBGM、助言使用時の「扇の音」。
