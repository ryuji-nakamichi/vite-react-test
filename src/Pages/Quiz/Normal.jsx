...
function Normal() {
  ...
  const handleOptionClick = (selectedIndex) => {
    if (selectedIndex === normalQuizData[quizIndex].answerIndex) {
      // 正解の場合の処理
  ...
  return (
    <div id="appWrapper" className="appWrapper">
      <div className="appContents">
        ...
        {normalQuizData[quizIndex] && (
          <div className="appQuizData">
            <p className="appLead">{normalQuizData[quizIndex].question}</p>
            <p className="appLead">正解を選択してください。</p>
            <div className="appQuizButtonListContaier">
              {normalQuizData[quizIndex].options.map((option, index) => (
